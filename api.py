...
LOGGER = logging.getLogger("PorkBunAPI")
LOGGER.setLevel(logging.INFO)
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
for handler, level in ((logging.FileHandler("porkbun.log"), logging.DEBUG),
...
class PorkBunAPI():
    def __init__(self, config_file="config.json", domain=""):
        LOGGER.info(f"{domain} initialized using {os.path.abspath(config_file)}")
        self.domain = domain
    ...
    def get_records(self):
        LOGGER.info(f"Getting records for {self.domain}")
    ...
    def create_record(self, type, name, content, ttl):
        LOGGER.info(f"Creating {type} record for {name+'.' if name else ''}{self.domain} pointing to {content}")
    ...
    def read_record(self, type, name):
        LOGGER.info(f"Reading {type} record of {name+'.' if name else ''}{self.domain}")
    ...
    def update_record(self, type, name, content, ttl):
        LOGGER.info(f"Updating {type} record of {name+'.' if name else ''}{self.domain} pointing to {content}")
    ...
    def delete_record(self, type, name):
        LOGGER.info(f"Deleting {type} record from {name+'.' if name else ''}{self.domain}")
